...
class MQTTManager(val connectionParams: MQTTConnectionParams, val context: Context, val uiUpdater: UIUpdaterInterface?) {
    ...
    init {
        ...
        client.setCallback(object: MqttCallbackExtended {
            ...
            override fun messageArrived(topic:String, mqttMessage: MqttMessage) {
                ...
                uiUpdater?.payloadMes(mqttMessage.toString())
    ...
    fun publish(crrtopic:String,message:String){
        try
        {
            var msg =   message
            var tpc =   crrtopic
            //client.publish(this.connectionParams.topic,msg.toByteArray(),0,false,null,object :
             client.publish(tpc.toString(),msg.toByteArray(),1,false,null,object :
                IMqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken?) {
                    Log.w("Mqtt", "Publish Success!")
                    uiUpdater?.updateStatusViewWith("Published to Topic"+tpc.toString())
                }
